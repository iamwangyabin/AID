...
def rotate_queries_or_keys(x, pos, n_registers, has_cls_first):
    ...
    assert dim % 2 == 0, "Embedding dimension must be a multiple of 2"
...
class RoPEAttention(nn.Module):
    ...
    def forward(
        ...
        batch_size, n_tokens, dim = x.size()
        ...
        start = 0
        qd = rotate_queries_or_keys(q[..., start : start + self.d_dim], d_mask, self.n_registers, self.has_cls_first)
        kd = rotate_queries_or_keys(k[..., start : start + self.d_dim], d_mask, self.n_registers, self.has_cls_first)
        start += self.d_dim
        qh = rotate_queries_or_keys(q[..., start : start + self.h_dim], h_mask, self.n_registers, self.has_cls_first)
        kh = rotate_queries_or_keys(k[..., start : start + self.h_dim], h_mask, self.n_registers, self.has_cls_first)
        start += self.h_dim
        qw = rotate_queries_or_keys(q[..., start : start + self.w_dim], w_mask, self.n_registers, self.has_cls_first)
        kw = rotate_queries_or_keys(k[..., start : start + self.w_dim], w_mask, self.n_registers, self.has_cls_first)
        start += self.w_dim

        if start < self.head_dim:
            q = torch.cat([qd, qh, qw, q[..., start:]], dim=-1)
            k = torch.cat([kd, kh, kw, k[..., start:]], dim=-1)
        else:
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        qkv = self.qkv(x).reshape(batch_size, n_tokens, 3, self.num_heads, dim // self.num_heads)
