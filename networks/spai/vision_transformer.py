...
class VisionTransformer(nn.Module):
    def __init__(
        ...
        if use_abs_pos_emb:
            self.cls_pos_embed = nn.Parameter(torch.zeros(1, num_patches + 1, embed_dim))
        else:
        ...
        self.cls_norm = nn.Identity() if use_mean_pooling else norm_layer(embed_dim)
        ...
        if self.use_intermediate_layers:
            ...
            self.intermediate_fc_norm = nn.ModuleList(
                [norm_layer(embed_dim) if use_mean_pooling else None for _ in self.intermediate_layers]
            )
        else:
            cls_head_embed_dim = embed_dim

        self.head = nn.Linear(cls_head_embed_dim, num_classes) if num_classes > 0 else nn.Identity()

        if self.pos_embed is not None:
            self._trunc_normal_(self.pos_embed, std=0.02)
        if self.cls_pos_embed is not None:
            self._trunc_normal_(self.cls_pos_embed, std=0.02)
        self._trunc_normal_(self.cls_token, std=0.02)
        self._trunc_normal_(self.actual_cls_token, std=0.02)
        if num_classes > 0:
    ...
    def forward_features(self, x):
        ...
        if self.use_intermediate_layers:
            ...
            if self.return_features:
                ...
                return x
            for i in range(len(intermediate_features)):
                intermediate_features[i] = intermediate_features[i][:, 1:].mean(dim=1)
...
def build_vit(config):
    model = VisionTransformer(
        ...
        use_intermediate_layers=config.MODEL.VIT.USE_INTERMEDIATE_LAYERS,
        intermediate_layers=config.MODEL.VIT.INTERMEDIATE_LAYERS,
        return_features=config.MODEL.VIT.USE_INTERMEDIATE_LAYERS,
    )
